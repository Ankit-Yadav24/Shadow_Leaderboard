...
const CurrentUserCard = ({ currentUser }) => {
  const getSubjectScore = (subjectType) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-[200px] flex items-center gap-3 px-4">
        <div
          className="w-8 h-8 rounded-[18px] overflow-hidden border-2 flex-shrink-0"
          style={{
            background:
              "linear-gradient(0deg, #6EDAEB, #6EDAEB), linear-gradient(0deg, #D7BFE7, #D7BFE7)",
            borderColor: "var(--q3-stroke-normal)",
          }}
        >
          {currentUser.userId.profilePicture ? (
            <img
              ...
              onError={(e) => {
                console.log(
                  `Profile image failed to load for current user: ${currentUser.userId.name}`
                );
          ...
          <div
            className={`w-full h-full bg-gradient-to-br from-blue-400 to-blue-600 dark:from-blue-600 dark:to-blue-800 flex items-center justify-center text-sm font-semibold text-white fallback-avatar ${
              currentUser.userId.profilePicture ? "hidden" : ""
            }`}
          >
