...
const ActionBar = ({ onBackClick }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const shouldBeScrolled =
        window.scrollY >
        actionBarRef.current.offsetTop + actionBarRef.current.offsetHeight;
  ...
  const bgColor =
    theme === "dark" ? "rgb(28, 34, 40,0.7)" : "rgba(255, 255, 255, 0.7)";
  const buttonBgColor =
    theme === "dark" ? "rgba(70, 130, 230, 0.1)" : "rgba(0, 88, 198, 0.06)";
  const textColor = theme === "dark" ? "#FFFFFF" : "#1D2933";

  return (
    <div className="flex justify-center w-full">
      {isScrolled && (
        <div className="h-[88px] max-w-[1176px] w-full mx-auto"></div>
      )}
      ...
      <div
        ...
        className={`w-full max-w-[1176px] z-50 transition-all duration-300 ease-out
                   ${
                     isScrolled
                       ? "fixed top-0 border-b shadow-sm rounded-b-2xl animate-slideDown"
                       : ""
                   } 
                   ${
      ...
      >
        <div
          className={`w-full px-6 transition-all duration-300 ease-out
                       ${
